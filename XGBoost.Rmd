...
#names(bioclim_pak) <- c("Annual Mean Temperature",
#  "Mean Diurnal Range",
#  "Isothermality",
...
# we want to use binary classification with logistic regression, which outputs a probability
...
train <- function(
  ...
  message(paste("avg train-logloss:", mean(model[["evaluation_log"]][["train_logloss"]])))
...
ggplot() + 
  ...
  geom_sf(data = sf::st_as_sf(border_pak),
   ...
   ggtitle(paste("Oberserved and predicted occurrence of", sp, "in pakistan", "\ntrain-logloss:", mean(model[["evaluation_log"]][["train_logloss"]])))
```

As logloss is very high and the map
...
Let's repeat this wiht improved parameters:
nrounds = 100
...
model <- train(data, nrounds = 100, save_model = FALSE)
...
model <- train(data, nrounds = 1000, save_model = FALSE)
...
```{r training parameter, eval=F, include=F}
rm(xgboost_params)
...
```{r loop, message=F, warning=FALSE, error=FALSE}
chunk_start <- proc.time()

rm(l_logloss)
l_logloss <- data.frame(species = character(),
                        logloss = numeric())
...
for(sp in species)
{
  ...
  l_logloss[nrow(l_logloss) + 1,] = c(sp, as.numeric(model[["evaluation_log"]][["train_logloss"]]) )
  ...
  ggplot() + 
    ...
    geom_sf(data = sf::st_as_sf(border_pak),
            fill = NA, show.legend=FALSE) +
    ggtitle(paste("Oberserved and predicted occurrence of", sp, "in pakistan", paste("\ntrain_logloss:", as.numeric(mean(model[["evaluation_log"]][["train_logloss"]])))))
...
x <- merge(l_logloss, species_nsamples, by="species")
x$logloss = as.numeric(x$logloss)

ggplot(x, aes(x = n_samples, y = logloss)) +
  geom_point() #+ ylim(0,1)
