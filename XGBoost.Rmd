...
**Are there presence and absence points inside the modeling_data ?** Check if the species column has species names (=presence) and NA (=absence) values. The data was combined by rbind, so the species_presence data should be at the top and species_absence data at the bottom of the dataframe. Let's check this by using the head() and tail() functions, which print us the first/last 6 rows.
```{r Data overview 1}
