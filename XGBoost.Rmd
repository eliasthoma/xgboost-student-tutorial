...
```{r Loading libaries, results='hide'}
...
# use this line to select a subset of species to speed up runtime during development
modeling_data = modeling_data %>% filter( species %in% c(species_nsamples$species[1:50], NA) )
...
for(sp in (modeling_data %>% distinct(species) %>% filter(!is.na(species)))$species)
{
  ...
  xgb_model <- xgb.train(data = training_data,
                       ...
                       gamma = 0.3, # Minimum loss reduction, default = 0
...
for(sp in (modeling_data %>% distinct(species) %>% filter(!is.na(species)))$species)
{
  ...
  # prediciton_raster = terra::clamp(prediction_raster, lower=10, values=FALSE)
  
  terra::writeRaster(prediction_raster, paste("out/", sp, "/" ,sp, ".tif", sep = ""), overwrite=TRUE)
  #terra::writeRaster(prediciton_raster$lyr1, "test.tif", overwrite=TRUE, datatype="FLT8S" )
  
  plot(prediction_raster)
  
  ggplot() + geom_spatraster(data=prediction_raster) +
    #scale_fill_binned() +
    #scale_alpha() + 
    #scale_fill_gradient(low="green", high="red") + 
    geom_sf(data = sf::st_as_sf(border_pak), fill=NA, show.legend=FALSE)
  
  #gsave(paste(sp, ".png", sep=""), path=paste("out/",sp, sep=""))
  
  # TODO: clean up in for loop and at end of chunk
}
```
...
# TODO: show results of two species here and explain those
```
...
for(sp in (modeling_data %>% distinct(species) %>% filter(!is.na(species)))$species)
{
  ...
  r = terra::classify(r, m, include.lowest = TRUE)
